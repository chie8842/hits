...
class hits_algorithm(obj):
    def __init__(self, iter=10, early_stop=False, tol=0):
        ...
        self.fin_flag = False
        self.links = links
        self.from_links_with_hub_scores = encoder(links[0], 'from_link')
        self.to_links_with_auth_scores = encoder(links[1], 'to_link')
    ...
    def train(self, data, from_links_col='from_links', to_links_col='to_links'):
        self.hub_scores, self.hubs = init_scores(data[from_links_col])
        self.auth_scores, self.auths = init_scores(data[to_links_col])
        for i in range(self.iter):
            if ! fin_flag:
                update_auths_and_hubs()
            else:
                break
        return self.hub_scores, self.auth_scores

    def init_scores(self, link_list, link_type):
        le = preprocessing.LabelEncoder()
        encoded = le.fit_transform(str_list)
